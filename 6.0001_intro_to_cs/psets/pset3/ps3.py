...
# Name          : PBa
# Collaborators : N/A
# Time spent    : Start = 1Sept, End = TBD
...
def get_word_score(word, n):
    ...
    for letter in word:
        points_letter_sum += SCRABBLE_LETTER_VALUES[letter]
    
    word_length = len(word)
    points_length_word = 7 * word_length - 3 * (n-word_length)
    
    # Minimum Point
    if points_length_word < 1:
        points_length_word = 1
    
    return points_letter_sum * points_length_word
...
def is_valid_word(word, hand, word_list):
    ...
    # Check whether each letter is present and available on hand
    # Disregards the wildcard '*'
    word_letter_count = get_frequency_dict(word)
    for letter in word_letter_count.keys():
        if letter not in hand.keys():
            return False
        if hand[letter] - word_letter_count[letter] < 0:
            return False
        if letter == '*' : continue
    ...
    if '*' in word:
        for vowel in VOWELS:
            ...
            if temp_word in word_list:
                # Here we know that there exist a valid word using a wildcard            
                return True
        # Iterated all vowels but still no match
        return False
    else:
        return True
...
def substitute_hand(hand, letter):
    ...
    if letter not in hand.keys():
        ...
        for char in hand.keys():
            # Remove already available characters for replacement pool
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    total_score = 0
    is_substituted_flag = False
    is_replayed_flag = False
    while hand_number > 0:

        # TEST CASES
        #hand = get_frequency_dict('aci*prt')
        
        hand = deal_hand(HAND_SIZE)
        print("Current Hand: ", end="")
        display_hand(hand)
        
        if is_substituted_flag == False and is_replayed_flag == False:
